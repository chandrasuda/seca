#!/usr/bin/env python3
"""Evaluate a DistilTrainer SDFT checkpoint on APPS (Pass@k).

DistilTrainer saves checkpoints in HuggingFace format (full model directory),
not the raw .pt format used by the older eval.py.  This script loads the
checkpoint directly via vLLM for fast batch generation, then runs the
existing APPS sandbox executor to compute Pass@k.

Usage:
  # Evaluate latest checkpoint
  python scripts/eval_sdft_distil.py \
      --checkpoint checkpoints/sdft_distil/checkpoint-200

  # Evaluate with more samples for reliable Pass@k
  python scripts/eval_sdft_distil.py \
      --checkpoint checkpoints/sdft_distil \
      --n-samples 20 --max-problems 200

  # Quick smoke test
  python scripts/eval_sdft_distil.py \
      --checkpoint checkpoints/sdft_distil/checkpoint-100 \
...
def _find_best_checkpoint(checkpoint_dir: str) -> str:
    """If checkpoint_dir has sub-directories like checkpoint-NNN, return the latest."""
    p = Path(checkpoint_dir)
    if (p / "config.json").exists():
        return str(p)  # it's already a leaf checkpoint dir
    candidates = sorted(
        [d for d in p.iterdir() if d.is_dir() and d.name.startswith("checkpoint-")],
        key=lambda d: int(d.name.split("-")[-1]),
    )
    if not candidates:
        return str(p)  # fall back to the root (vLLM will fail if invalid)
    latest = candidates[-1]
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(
        description="Evaluate SDFT checkpoint (HF format) on APPS Pass@k"
    )
    p.add_argument(
        "--checkpoint",
        required=True,
        help="Path to HF checkpoint directory (DistilTrainer output)",
    )
    # Data
    p.add_argument(
        "--split",
        default="test",
        help="APPS split to evaluate on: train | test | all (default: test)",
    )
    p.add_argument(
        "--difficulty",
        default=None,
    ...
    p.add_argument(
        ...
        type=float,
        default=0.9,
        help="vLLM GPU memory fraction (high is OK: no training during eval)",
    )
    p.add_argument(
        "--max-model-len",
        type=int,
...
def main() -> None:
    ...
    out_file = out_dir / f"eval_sdft_{ckpt_name}.json"
